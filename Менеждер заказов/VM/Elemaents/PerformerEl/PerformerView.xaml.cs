...
{
    public partial class PerformerView : UserControl, IDisposable
    {
        private MODBEntities db;
        private PerformerViewModel model;

        public PerformerView(MODBEntities db, int id = 0)
        {
            InitializeComponent();
            this.db = db;
            model = new PerformerViewModel(db.Performers.Where(p => p.Id == id).FirstOrDefault(), db);
